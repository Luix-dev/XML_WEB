<?xml version="1.0"?>
<xsl:stylesheet xmlns="" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" exclude-result-prefixes="xhtml">
	<xsl:template match="x">
    <xsl:call-template name="fsin"/>
	</xsl:template>
	<xsl:template match="sqrt">
		<p>sqrt(<xsl:value-of select="."/>) = <xsl:call-template name="sqrt"><xsl:with-param name="x" select="."/></xsl:call-template></p>
	</xsl:template>
	<xsl:template match="cos">
		<p>cos(<xsl:value-of select="."/>) = <xsl:call-template name="cos"><xsl:with-param name="x" select="."/></xsl:call-template></p>
	</xsl:template>
	<xsl:template match="sin" name="fsin">
		<!--<p>sin(<xsl:value-of select="."/>) = --><xsl:call-template name="sin"><xsl:with-param name="x" select="."/></xsl:call-template><!--</p>-->
	</xsl:template>
	<xsl:template match="log">
		<p>log(<xsl:value-of select="."/>) = <xsl:call-template name="log"><xsl:with-param name="x" select="."/></xsl:call-template></p>
	</xsl:template>
